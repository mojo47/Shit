...
*Setup instructions for a Gitian build of Shit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Shit
...
- [Building Shit Core](#building-shit-core)
...
Clone the git repositories for shit and Gitian.
...
in the shit repository under 'Fetch and create inputs' to install sources which require
...
To build Shit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the shit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/shit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/shit-project/shit
...
For example:
```bash
URL=https://github.com/thrasher-/shit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit shit=${COMMIT} --url shit=${URL} ../shit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit shit=${COMMIT} --url shit=${URL} ../shit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit shit=${COMMIT} --url shit=${URL} ../shit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the shit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../shit/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/shit
SIGPATH=/some/root/path/shit-detached-sigs

./bin/gbuild --url shit=${BTCPATH},signature=${SIGPATH} ../shit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/shit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/shit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/shit-osx-build.assert
```
...
[shit-project/gitian.sigs.ltc](https://github.com/shit-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
