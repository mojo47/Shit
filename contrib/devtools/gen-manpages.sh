...
LITECOIND=${LITECOIND:-$SRCDIR/shitd}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/shit-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/shit-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/shit-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
SHTVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
